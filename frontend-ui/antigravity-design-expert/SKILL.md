...
- **Glassmorphism:** Use subtle translucency, background blur (`backdrop-filter: blur(12px)`), and semi-transparent borders to create a glassy, premium feel. See [ui-ux-pro-max](../ui-ux-pro-max/SKILL.md) for supplementary visual style opinions.
...
- **Responsiveness & Layout:** Always write modular, reusable components. Integrate with [tailwind-patterns](../tailwind-patterns/SKILL.md) for utility styling and [frontend-dev-guidelines](../frontend-dev-guidelines/SKILL.md) for scalable frontend code.
- **Accessibility:** Ensure all animations are disabled for users with `prefers-reduced-motion: reduce`.
- **Performance:** Prioritize performance: Use `will-change: transform` for animated elements to offload rendering to the GPU. Do not animate expensive properties like `box-shadow` or `filter` continuously.

## Related Skills
- [ui-ux-pro-max](../ui-ux-pro-max/SKILL.md) - Style matching, aesthetics, and UX details.
- [frontend-dev-guidelines](../frontend-dev-guidelines/SKILL.md) - React standards, structure, and quality guidelines.
- [tailwind-patterns](../tailwind-patterns/SKILL.md) - Direct Tailwind classes, configurations, and themes.
